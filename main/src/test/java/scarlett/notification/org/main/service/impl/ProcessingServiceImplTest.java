...
import scarlett.notification.org.main.sender.impl.ChannelFactoryImpl;
import scarlett.notification.org.main.sender.impl.PushSenderAdapterImpl;
import scarlett.notification.org.main.sender.impl.SmsSenderAdapterImpl;
...
import scarlett.notification.org.main.template.TemplateEngineImpl;
import scarlett.notification.org.main.template.TemplateExecutorImpl;
...
class ProcessingServiceImplTest {
    ...
    private final DecoratedSenderAdapter proxy = Mockito.mock(DecoratedSenderAdapter.class);
    ...
    private final DeliveryDispatcher deliveryDispatcher = new DeliveryDispatcherImpl(
            new ChannelFactoryImpl(Map.of("SMS", smsSender, "PUSH", pushSender), proxy));
    ...
    public void test() {
        ...
        Mockito.verify(smsSender, Mockito.times(1))
               .send(Mockito.argThat(message ->
                                             message.getBody().equals("Hello World") && message.getSubject()
                                                                                               .equals("Title")
                                    ));
        Mockito.verify(pushSender, Mockito.times(1)).send(any());
    }
